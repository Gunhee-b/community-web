...
## 📋 프로젝트 개요
250명 규모의 '통찰방' 커뮤니티를 위한 투표 및 철학챗 웹 애플리케이션입니다.
...
- **모임 유형 구분** (v2.6.0)
  - **정기 모임**: 매주 일정한 요일과 시간에 진행
  - **즉흥 모임**: 특정 날짜에 일회성으로 진행
    - 취미 모임: 취미 활동 중심 모임
    - 토론 모임: 토론 및 철학적 논의 모임
- 커피 또는 술 모임 선택
- **카카오톡 오픈채팅 링크 연동**
...
6. **offline_meetings** - 오프라인 모임
   ...
   - `meeting_type`: 모임 유형 (regular/casual) (v2.6.0)
   - `casual_meeting_type`: 즉흥 모임 세부 유형 (hobby/discussion) (v2.6.0)
   - `recurrence_day_of_week`: 정기 모임 요일 (0-6) (v2.6.0)
   - `recurrence_time`: 정기 모임 시간 (v2.6.0)
...
- `/signup` - 회원가입
- `/` - 메인 홈 (커뮤니티 소개)
- `/meetings` - 철학챗 목록 (조회만 가능)
- `/meetings/:id` - 철학챗 상세 (조회만 가능, 참가는 로그인 필요)
...
### v2.6.0 (2025-02-04)
- ✨ **정기 모임 및 즉흥 모임 기능 추가**
  - 정기 모임: 매주 정해진 요일과 시간에 자동 진행
  - 즉흥 모임: 취미 모임과 토론 모임으로 세분화
  - 단순화된 필터 (즉흥 모임, 정기 모임, 지난 모임)
  - 지난 모임은 관리자만 조회 가능
  - 모임 목록 및 상세 페이지 UI 개선
  - 색상 코딩으로 모임 유형 구분
- 🗄️ **데이터베이스 스키마 확장**
  - meeting_type, casual_meeting_type ENUM 추가
  - recurrence_day_of_week, recurrence_time 컬럼 추가
  - 제약 조건으로 데이터 무결성 보장
  - 기존 데이터 자동 마이그레이션 (hobby로 설정)
- 🎨 **UI/UX 개선**
  - 2개 메인 탭 (즉흥 모임, 정기 모임)
  - 관리자 전용 탭 (지난 모임)
  - 권한 기반 UI 렌더링
