...
### 읽은 알림
- **목록에 표시되지 않음** (자동으로 숨김)
- 하지만 localStorage에는 남아있어 중복 알림 방지
- 전체 삭제 시 완전히 삭제됨
...
- addNotification(notification): 새 알림 추가
  * messageId 기반 중복 체크 (같은 메시지는 한 번만 알림)
  * 이미 존재하는 알림은 생성하지 않음
...
1. ✅ 브라우저 권한 필요 없음
2. ✅ 모든 브라우저에서 작동
3. ✅ **중복 알림 완벽 방지** (messageId 기반)
4. ✅ 읽은 알림 자동 숨김 처리
5. ✅ 알림 관리 기능 (읽음, 삭제)
6. ✅ 클릭으로 페이지 이동
7. ✅ localStorage 저장으로 영구 보관
...
**중요 필드:**
- `id`: 알림 자체의 고유 ID (삭제/읽음 처리용)
- `messageId`: 원본 메시지의 ID (중복 알림 방지용)
  - 같은 `messageId`를 가진 알림은 한 번만 생성됨
  - 읽은 알림도 배열에 유지되어 중복 체크에 사용
