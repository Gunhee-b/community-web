-- Fix: Handle invitation_codes foreign key constraint when deleting users
-- The ON DELETE SET NULL is not working properly, so we need to manually set it before deletion
...
BEGIN
  ...
  -- CRITICAL FIX: Manually set invitation_codes.used_by to NULL before deleting user
  ...
  -- This will CASCADE delete social_connections, posts, votes, etc.
